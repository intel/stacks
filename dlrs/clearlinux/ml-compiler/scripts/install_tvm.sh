...
export PROJECT=tvm
export PROJECT_GIT_URL=https://github.com/dmlc/${PROJECT}.git
#export PROJECT_GIT_HASH=25bad44
export PROJECT_VERSION=v0.6
export PROJECT_BUILD_DIR="${PROJECT:?}"/build/
...
get_project() {
  # clone the repo
  if [ ! -d ./${PROJECT} ]; then
      git clone --recursive -j"$N_JOBS" ${PROJECT_GIT_URL} \
      && cd "./${PROJECT}" && git checkout -b ${PROJECT_VERSION} \
      && cd ..
  fi
}
...
build() {
  ...
  cmake $CMAKE_ARGS ..
  make -j"$N_JOBS"
  echo "${PROJECT} build directory is: ${PROJECT_BUILD_DIR}"
}
