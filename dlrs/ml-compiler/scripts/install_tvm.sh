...
export CFLAGS="$CFLAGS -mtune=skylake-avx512 -m64 -pipe"
export CXXFLAGS="$CXXFLAGS -mtune=skylake-avx512 -m64 -pipe"
...
python_pkgs() {
  pip3 --no-cache-dir install numpy decorator \
	  Jupyter pytest ipython attrs tornado \
	  psutil xgboost nose-timer
}
...
build() {
  ...
  cd ..; cd python; python3 setup.py install --user; cd ..
}

cleanup() {
  # remove unneeded tvm directories
    echo "clean up unneeded files.."
    cd /tvm; dirs=(*/); needed_dirs=( nnvm/ vta/ build/ python/ )
