...
#Work in a copy
ORIG_CONFIG_FILE="/opt/cassandra-pmem/conf/cassandra.yaml"
CONFIG_FILE="/opt/cassandra-pmem/conf/cassandra-template.yaml"
ORIG_JVM_OPTIONS_FILE="/opt/cassandra-pmem/conf/jvm-server.options"
JVM_OPTIONS_FILE="/opt/cassandra-pmem/conf/jvm-server.options-template"
...
function create_cassandra_yaml {
	...
	#Concurrent reads
        ...
        echo "concurrent_reads: $CASSANDRA_CONCURRENT_READS" | tee -a $CONFIG_FILE

	#Concurrent writes
        CASSANDRA_CONCURRENT_WRITES=${CASSANDRA_CONCURRENT_WRITES:-'32'}
        echo "concurrent_writes: $CASSANDRA_CONCURRENT_WRITES" | tee -a $CONFIG_FILE
...
if [ "$#" -eq 0 ] || [ "${1#-}" != "$1" ]; then
	set -- /opt/cassandra-pmem/bin/cassandra "$@"
fi
